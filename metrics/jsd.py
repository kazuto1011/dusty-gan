...
def entropy_of_occupancy_grid(pcs, resolution, in_sphere=False, batch_size=128):
    device = pcs.device
    epsilon = 1e-3
    bound = 0.5 + epsilon
    ...
    inds = []
    for i in tqdm(range(0, B, batch_size), leave=False, desc="JSD: voting"):
        inds_ = []
...
@torch.no_grad()
def compute_jsd(pcs_gen, pcs_ref, resolution=28, batchsize=128):
    _, gen_grid_var = entropy_of_occupancy_grid(pcs_gen, resolution, True, batchsize)
    _, ref_grid_var = entropy_of_occupancy_grid(pcs_ref, resolution, True, batchsize)
